package nio.mina.file.filter.codec.v2;
...
public class RetryFileDecoder implements ProtocolDecoder {
	...
	private static String	CONTEXT	=RetryFileDecoder.class.getName() + ".context";

	// 构造函数必须指定Charset和文本分隔符
	public RetryFileDecoder(Charset charset, String delimiter) {
	...
	private Context getContext(IoSession session) {
		...
		if(ctx == null){
			ctx=new Context(charset.newDecoder());
	...
	public static class Context {
		...
		Context(CharsetDecoder	decoder) {
