...
public class ZipEncryptAes {
	private final static Logger	log	=LoggerFactory.getLogger(ZipEncryptAes.class);
	...
	private void unZip(String directory, String zipFile) {
		try{
			...
			if(!f.mkdirs()){
				ExceptionUtil.newRuntimeException("file mkdirs fail:" + f.toString());
			}
	...
	private Key getKey(String keyPath) throws Exception {
		byte[] bytes=Files.toByteArray(new File(keyPath));
		SecretKeySpec dks=new SecretKeySpec(bytes,"AES");
		return dks;
	}
	...
	public void encryptZip(String srcFile, String destfile, String keyfile) throws Exception {
		...
		AssertUtil.isTrue(!f.getParentFile().exists() && !f.getParentFile().mkdirs(),"can not mkdirs parentFile:" + f.getParentFile().toString());
		AssertUtil.isTrue(!f.createNewFile(),"can not create file:" + f.toString());
		FileOutputStream fos=new FileOutputStream(f);
		...
		if(!temp.delete()){
			log.warn("file cannot be delete:{}",temp.toString());
		}
	}
	...
	public static void main(String args[]) throws Exception {
		long a=System.currentTimeMillis();// D:/Server/java/zip
		new ZipEncryptAes().encryptZip("D:/Server/java/zip/file","D:/Server/java/zip/file1.zip","D:/Server/java/zip/public.key");
		
		System.out.println(System.currentTimeMillis() - a);
		a=System.currentTimeMillis();
		
		new ZipEncryptAes().decryptUnzip("D:/Server/java/zip//file1.zip","D:/Server/java/zip/unzip1","D:/Server/java/zip/public.key");
		System.out.println(System.currentTimeMillis() - a);
	}
}