...
public class WaterMark {
	 ...
	 * @return 
	 */
	private int[] getPixArray(Image imageSrc, int startX, int startY, int w, int h) {
	 ...
	 *            默认值128。超过范围，按默认值进行。 
	 */
	private final void pressImage(String lightnessWaterImg, String darknessWaterImg, String targetImg, int startX,
			int startY, int x, int y, float alpha, float averageGray) {
		try {
			// 先判断亮水印和源文件的值是否为null，否则抛出异常  
			if (lightnessWaterImg == null ||  "".equals(lightnessWaterImg) || targetImg == null || "".equals(targetImg)) {
	...
	public static void main(String[] args) {
		...
		waterMark.pressImage("c:/icon_tag.gif","c:/icon_tag.gif",
				"c:/1.png", 520, 500, 900, 800, 0.5f, 50);
